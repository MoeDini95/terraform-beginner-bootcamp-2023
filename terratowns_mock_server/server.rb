...
# This is a ruby class that includes validations from an active record. 
...
class Home
  ...
  #The Prod Terratowns server is railes and uses 
  # very similar and in most cases ideitcal validations 
  ...
  # content ver has to be an integer 
  # we will make sure it is an incremental ver in the controller 
...
class TerraTownsMockServer < Sinatra::Base
  ...
  def find_user_by_bearer_token
    ...
    #Check if the bearer token in blank/missing, if so throw an error. 
    if auth_header.nil? || !auth_header.start_with?("Bearer ")
      error 401, "a1000 Failed to authenicate, bearer token invalid and/or teacherseat_user_uuid invalid"
    end

    # Does the token match the one in our database?
    #If we cant find it, return an error or if it doesn't match 
  ...
  post '/api/u/:user_uuid/homes' do
    ...
    # Puts will print to the temrinal similar to a print or console.log
    puts "# create - POST /api/homes"

    # a begin/rescue is a try/catch, if an error occures, rescue it. 
    begin
      # Sinatra doe snot auto part json bodys as params 
    ...
    # return the errors 
    unless home.valid?
      @ return the erros messages back at json
    ...
    # which is justa global variable 
    $home = {
      uuid: uuid,
      name: name,
      town: town,
  ...
  get '/api/u/:user_uuid/homes/:uuid' do
    ...
    content_type :json
    # Does the uuis for the hoe match the one in our database 
